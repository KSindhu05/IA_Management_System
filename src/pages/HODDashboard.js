...
const HODDashboard = ({ isSpectator = false, spectatorDept = null }) => {
    ...
    return (
        <div className={styles.dashboardContainer} style={isSpectator ? { gridTemplateColumns: '1fr', height: '100%' } : {}}>
            {!isSpectator && (
            ...
            <main className={styles.mainContent} style={isSpectator ? { padding: '1rem', height: 'auto', overflow: 'visible' } : {}}>
                {!isSpectator && (
                    <header className={styles.topHeader}>
                        <div className={styles.headerLeft}>{activeTab === 'overview' ? (<div><h1 className={styles.welcomeTextBig}>Hello, {user?.username}</h1><p className={styles.subtitle}>Head of {departments.find(d => d.id === selectedDept)?.name} | HOD</p></div>) : (<h1>{menuItems.find(m => m.active)?.label}</h1>)}</div>
